# Base class import
from RL_base import RLBase


class ExpectedSARSA(RLBase):
    """ Expected SARSA algorithm with constant step size following an epsilon-greedy policy """

    def _update_Q(self, old_state, old_action, reward, new_state, new_action):
        """
        Updates the state-action-value function.

        Parameters
        ----------
        old_state : int
            The old state in which the first action was taken.
        old_action : int
            The action taken in the old state.
        reward : float
            The reward received from the old state and action transition.
        new_state : int
            The state which the old state-action pair led to.
        new_action : int
            The action to take in the new state.

        """
        # Get the number of actions in the state
        no_actions = len(self.Q[new_state].keys())

        # The probability for the greedy action is: 1 - epsilon + epsilon / no. actions in state
        prob_greedy = 1 - self.epsilon + self.epsilon / no_actions

        # The probability for all the non-greedy actions are: epsilon / no. actions in state
        prob_non_greedy = self.epsilon / no_actions

        # Find the greedy action in the new state
        greedy_action = self._get_greedy_action(new_state)

        # Calculate the expected value of the new state-action pairs
        expected_value = 0
        for key in self.Q[new_state].keys():
            if key == greedy_action:
                expected_value += prob_greedy * self.Q[new_state][key]
            else:
                expected_value += prob_non_greedy * self.Q[new_state][key]

        # Perform the update
        self.Q[old_state][old_action] = self.Q[old_state][old_action] + \
                                        self.step_size * \
                                        (reward +
                                         self.discount_rate *
                                         expected_value -
                                         self.Q[old_state][old_action])

    def _run_one_episode(self, old_state):
        """
        Runs one episode.

        Parameters
        ----------
        old_state : int
            The starting state for the agent.

        Returns
        -------
        The total reward and the number of environment samples as float/int and ints.

        """
        # Initialize metrics
        current_episode_reward = 0
        current_episode_samples = 0

        # Choose an action
        old_action = self._get_action_epsilon_greedy(old_state)

        # Loop until the episode is done
        done = False
        while not done:
            # Take the old action
            new_state, reward, done, _ = self.env.step(old_action)

            # Choose an action from the new state
            new_action = self._get_action_epsilon_greedy(new_state)

            # Update Q
            self._update_Q(old_state, old_action, reward, new_state, new_action)

            # Update the old state and action
            old_state, old_action = new_state, new_action

            # Update the metrics
            current_episode_reward += reward
            current_episode_samples += 1

        return current_episode_reward, current_episode_samples
